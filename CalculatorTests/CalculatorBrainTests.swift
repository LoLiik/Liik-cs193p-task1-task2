...
class CalculatorBrainTests: XCTestCase {
    ...
    func testEvaluation1() {
        var testBrain = CalculatorBrain()
        testBrain.evaluationChain = [.number(3),.operation("+"),.variable("x"),.operation("=")]
        testBrain.namedVariableOperands = ["m":4]
        let noVariableRelult = testBrain.evaluate(using: testBrain.namedVariableOperands)
        XCTAssertEqual(noVariableRelult.result,3)
    }
    
    func testEvaluationX() {
        var testBrain = CalculatorBrain()
        testBrain.evaluationChain = [.number(3),.operation("+"),.variable("x"),.operation("=")]
        testBrain.namedVariableOperands = ["x":3]
        let noVariableRelult = testBrain.evaluate(using: testBrain.namedVariableOperands)
        XCTAssertEqual(noVariableRelult.result,6)
    }
    
    func testEvaluate2(){
        var testBrain = CalculatorBrain()
        testBrain.evaluationChain = [.number(3),.operation("+"),.variable("x"),.operation("=")]
        testBrain.namedVariableOperands = ["m":4, "x":12]
        let variableRelult = testBrain.evaluate(using: testBrain.namedVariableOperands)
        XCTAssertEqual(variableRelult.result,15.0)
    }
    
    func testM1(){
        var testBrain = CalculatorBrain()
        testBrain.evaluationChain = [.number(3),.operation("+"),.variable("M"),.operation("=")]
        let variableRelult = testBrain.evaluate(using: testBrain.namedVariableOperands)
        XCTAssertEqual(variableRelult.result, 3)
        XCTAssertEqual(testBrain.namedVariableOperands, [String:Double]())
    }
    ...
    var testBrain = CalculatorBrain()
        ...
        testBrain.namedVariableOperands = ["M":4, "x":12]
        let variableRelult = testBrain.evaluate(using: testBrain.namedVariableOperands)
        XCTAssertEqual(variableRelult.result,7.0)
    }
